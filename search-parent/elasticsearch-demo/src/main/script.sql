...
insert into menu(pid,menuName,menuDesc,`link`,`order`)values((select a.menuId from (select menuId from menu where menuName='搜索')a),'es搜索','es搜索','/admins/search/elasticsearch/list',2);
